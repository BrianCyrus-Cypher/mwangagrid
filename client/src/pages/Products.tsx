...
function ProductCard({
  ...
  return (
    <Card className="overflow-hidden flex flex-col group border-0 shadow-md card-hover">
      {/* Image */}
      <div
        className="relative h-36 sm:h-48 bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-800 dark:to-slate-900 overflow-hidden flex items-center justify-center cursor-pointer"
        onClick={onView}
      ...
      <div className="p-5 flex-1 flex flex-col min-w-0">
        ...
        <div className="flex items-center gap-1 mb-3">
          {[...Array(5)].map((_, i) => (
            <Star
              key={i}
              className={`w-3.5 h-3.5 ${i < 4 ? "fill-amber-400 text-amber-400" : "text-muted"}`}
            />
          ))}

...
export default defineConfig({
  ...
  dbCredentials: {
    url:
      ...
      "postgresql://user:password@localhost:5432/mwanga_grid",
  },
  verbose: true,
  strict: true,
});
...
export const paymentTransactions = pgTable(
  ...
  table => ({
    orderIdIdx: uniqueIndex("idx_payment_transactions_order_id").on(
      table.orderId
    ),
    transactionIdIdx: uniqueIndex("idx_payment_transactions_id").on(
      table.transactionId
    ),
  })
);
    ...
    if (record.requestCount >= MAX_REQUESTS) {
      return res.status(429).json({
        ...
        retryAfter: Math.ceil(
          (record.windowStart.getTime() + RATE_LIMIT_WINDOW - now.getTime()) /
            1000
        ),
      });
    }

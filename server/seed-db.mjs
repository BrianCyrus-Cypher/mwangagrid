...
const products = [
  {
    ...
    description: 'High-speed dual-band router with 1200 Mbps combined speed, perfect for homes and small offices.',
    category: 'routers',
    price: 4500,
    discountPrice: 3999,
    stock: 25,
  ...
  },
  {
    ...
    description: '48-port managed network switch with advanced security features for enterprise networks.',
  ...
  },
  {
    ...
    description: 'High-resolution dome camera with night vision, IP67 weatherproof rating, and motion detection.',
  ...
  },
  {
    ...
    description: 'Professional-grade Cat6 cable for high-speed data transmission up to 10 Gbps.',
    category: 'cables',
    price: 2200,
    discountPrice: null,
    stock: 50,
  ...
  },
  {
    ...
    description: 'Enterprise-grade WiFi 6 access point with 2.4/5 GHz bands and mesh capability.',
    category: 'routers',
    price: 12000,
    discountPrice: 10500,
    stock: 12,
  ...
  },
  {
    ...
    description: 'Ultra HD turret camera with advanced AI detection, perfect for commercial surveillance.',
    category: 'cctv_cameras',
    price: 15000,
    discountPrice: 13500,
    stock: 10,
  ...
  },
  {
    ...
    description: 'Reliable 24-port managed switch with VLAN support for medium-sized networks.',
    category: 'network_switches',
    price: 16000,
    discountPrice: null,
    stock: 6,
...
for (const product of products) {
  await connection.execute(
    ...
    [product.name, product.description, product.category, product.price, product.discountPrice, product.stock, product.sku, product.warranty]
  );
}
...
const services = [
  {
    ...
    description: 'Professional CCTV system installation with camera placement, wiring, and configuration.',
    serviceType: 'cctv_installation',
  },
  {
    name: 'Internet Installation Service',
    description: 'High-speed internet connection setup with router configuration and network optimization.',
...
const servicePackages = [
  // CCTV Installation packages
  {
    ...
    tier: 'Basic',
    price: 15000,
    features: JSON.stringify(['2 cameras', 'Basic DVR', 'Installation', '6 months warranty']),
    duration: '1 month',
  },
  {
    serviceId: serviceIds[0],
    tier: 'Standard',
    price: 35000,
    features: JSON.stringify(['4 cameras', 'Advanced DVR', 'Cloud backup', 'Mobile app', '1 year warranty']),
    duration: '1 month',
  },
  {
    serviceId: serviceIds[0],
    tier: 'Premium',
    price: 65000,
    features: JSON.stringify(['8 cameras', 'Enterprise DVR', 'Cloud backup', 'Mobile app', '24/7 monitoring', '2 years warranty']),
    duration: '1 month',
  },
  ...
  },
  {
    ...
    tier: 'Standard',
    price: 12000,
    features: JSON.stringify(['50 Mbps speed', 'Managed router', 'WiFi optimization', 'Support']),
    duration: '1 month',
  },
  {
    serviceId: serviceIds[1],
    tier: 'Premium',
    price: 25000,
    features: JSON.stringify(['100 Mbps speed', 'Enterprise router', 'WiFi optimization', '24/7 support', 'Backup connection']),
    duration: '1 month',
  },
];
...
for (const order of orders) {
  const [result] = await connection.execute(
    ...
    [order.userId, order.orderNumber, order.status, order.totalAmount, order.deliveryLocation, order.paymentMethod, order.paymentStatus]
  );
...
const quotations = [
  {
    ...
    items: JSON.stringify([{ name: '4MP CCTV Dome Camera', quantity: 4, price: 7499 }]),
    totalAmount: 29996,
    status: 'pending',
  },
  {
    ...
    items: JSON.stringify([{ name: 'Cisco Catalyst 2960X Switch', quantity: 1, price: 28000 }]),
    totalAmount: 28000,
    status: 'approved',
  },
];
...
for (const quote of quotations) {
  await connection.execute(
    ...
    [quote.userId, quote.quotationNumber, quote.description, quote.items, quote.totalAmount, quote.status]
  );
}
...
for (const sub of subscriptions) {
  await connection.execute(
    ...
    [sub.userId, sub.servicePackageId, sub.status, sub.startDate, sub.renewalDate]
  );
}
...
for (const ticket of tickets) {
  await connection.execute(
    ...
    [ticket.userId, ticket.ticketNumber, ticket.subject, ticket.description, ticket.status, ticket.priority]
  );
}
